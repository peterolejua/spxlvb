...
The name "parameter explosion" comes from the introduction of **p+1 expansion parameters** $\alpha_1, \ldots, \alpha_p, \alpha_{p+1}$. The $+1$ comes from the global expansion parameter $\alpha_{p+1}$, not from an intercept. The first $p$ are per-coordinate: at each coordinate update $j$, the optimal $\alpha_j$ rescales all other variational parameters. The $(p+1)$-th is a global rescaling applied after each full coordinate sweep. When all expansion parameters equal 1, the algorithm reduces to standard VB.

The key arguments controlling the explosion are `mu_alpha` (prior means, length $p+1$) and `alpha_prior_precision` (shared prior precision, scalar), while `b_prior_precision` (length $p$) controls the spike-and-slab slab width separately. When `intercept = TRUE`, the intercept is computed after fitting on centered/scaled data --- it does not add a column to `X` or change the parameter dimensions.

## Package Architecture

The figure below shows how the package functions relate to each
other. Blue nodes are user-facing (exported) functions, orange
nodes are the C++ backend, and gray dashed nodes are deprecated
wrappers that forward to `tune_spxlvb()`.

```{r flowchart, echo=FALSE, out.width="100%", fig.cap="Function dependency graph for the spxlvb package."}
...
L_mat <- get_L(p = p, range = 2.0, smoothness = 1.5)

# Generate the data
sim_data <- matern_data_gen(
  seed_val = 42,
  n = n,
  p = p,
  pi_0 = pi_0,
...
## Note: mu_alpha has length p+1 (one per expansion parameter, including the
## global alpha_{p+1} -- not related to the intercept), while b_prior_precision
## has length p (one per regression coefficient in the slab component). These
## defaults are determined automatically from X, so you only need to specify
## them if you want non-default values.
fit <- spxlvb(
  X = sim_data$X,
  Y = sim_data$Y,
  mu_0 = initials$mu_0,
...
The \code{grid_search_spxlvb_fit} function evaluates models over a 2D grid based on the Evidence Lower Bound (ELBO) or a provided validation set.
...
opt_fit <- grid_search_spxlvb_fit(
  X = sim_data$X,
  Y = sim_data$Y,
